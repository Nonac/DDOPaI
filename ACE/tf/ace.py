...
from ACE.tf.ace_helpers import *
import tensorboard
...
class ConceptDiscovery(object):
  ...
  def create_patches(self, method='slic', discovery_images=None,
    ...
    '''
        ...
        But in imagenet, there are over 160 class h1==h2 or w1==w2 by slic, and make dataset, patches in [224,224], 
        not[3,224,224].

        '''

    for idx in range(len(dataset)):
      if dataset[idx].shape != (224,224,3):
        temp=np.reshape(dataset[idx],(224,224,1))
        dataset[idx]=np.concatenate([temp,temp,temp],2)
    for idx in range(len(patches)):
      if dataset[idx].shape != (224, 224, 3):
        temp = np.reshape(patches[idx], (224, 224, 1))
  ...
  def _extract_patch(self, image, mask):
    ...
    But in imagenet, there are over 160 class h1==h2 or w1==w2 by slic.
  ...
  def _return_gradients(self, images):
    ...
    for bn in self.bottlenecks:
      ...
      for i in range(len(acts)):
        bn_grads[i] = self.model.get_gradient(acts[i:i+1], [class_id], bn).reshape(-1)
      gradients[bn] = bn_grads
      # acts = get_acts_from_images(images, self.model, bn)
      # bn_grads = np.zeros((acts.shape[0], np.prod(acts.shape[1:])))
      # for i in range(len(acts)):
      #   bn_grads[i] = self.model.get_gradient(acts[i:i+1], [class_id], bn,images[i:i+1]).reshape(-1)
      # gradients[bn] = bn_grads
    return gradients
