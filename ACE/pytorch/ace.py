...
from ACE.pytorch.tcav import cav
from ACE.pytorch.ace_helpers import *
...
def make_model(settings,label_list,model):
    for name in settings.FEATHER_NAMES:
...
class ConceptDiscovery(object):
  ...
  def load_concept_imgs(self, concept, max_imgs=1000, other_dir=None):
    ...
    if other_dir is None:
      ...
      concept_dir = os.path.join(other_dir, concept)
    ...
    if self.data.shape[3]==1:
      gray_images = 0.33 * imgs[:,:,:,0] + 0.33 * imgs[:,:,:,1] + 0.33 * imgs[:,:,:,2]
      imgs = gray_images[:,:,:,np.newaxis]
    return imgs
  ...
  def  create_patches(self, method='slic', discovery_images=None,
    ...
    for idx in range(len(dataset)):
      # if dataset[idx].shape == tuple(self.image_shape):
      #    dataset[idx]=np.reshape(dataset[idx],tuple(self.image_shape+[1]))
      #elif dataset[idx].shape != tuple(self.image_shape+[3]):
      if dataset[idx].shape != tuple(self.image_shape+[3]):
        temp=np.reshape(dataset[idx],tuple(self.image_shape+[1]))
        dataset[idx]=np.concatenate([temp,temp,temp],2)
    for idx in range(len(patches)):
      # modified by nonac
      # 20231105
      if dataset[idx].shape == tuple(self.image_shape+[1]):
      #if dataset[idx].shape == tuple(self.image_shape):
        patches[idx]=np.reshape(patches[idx], tuple(self.image_shape+[1]))
      elif dataset[idx].shape != tuple(self.image_shape+[3]):
        temp = np.reshape(patches[idx], tuple(self.image_shape+[1]))
        patches[idx] = np.concatenate([temp, temp, temp], 2)
  ...
  def _return_superpixels(self, img, method='slic',
    ...
    for i in range(n_params):
      param_masks = []
      if method == 'slic':
        # modified by nonac 20231106
        segments = segmentation.slic(
            img, n_segments=param_dict['n_segements'][i], compactness=compactnesses[i],
            sigma=sigmas[i])
      elif method == 'watershed':
  ...
  def _extract_patch(self, image, mask):
    ...
    if patch.shape[2] == 1:
      if h1==h2:
        image = Image.fromarray((np.squeeze(patch[h1, w1:w2]) * 255).astype(np.uint8))
      elif w1==w2:
        image = Image.fromarray((np.squeeze(patch[h1:h2, w1]) * 255).astype(np.uint8))
      else:
        image = Image.fromarray((np.squeeze(patch[h1:h2, w1:w2]) * 255).astype(np.uint8))
    else:
      if h1==h2:
        image = Image.fromarray((patch[h1, w1:w2] * 255).astype(np.uint8))
      elif w1==w2:
        image = Image.fromarray((patch[h1:h2, w1] * 255).astype(np.uint8))
      else:
  ...
  def _patch_activations(self, imgs, bottleneck, bs=100, channel_mean=None, transform=None):
    ...
    if self.num_workers:
      ...
      for i in range(int(imgs.shape[0] / bs) + 1):
        ...
        # add by nonac
        if img_batch_tensor.shape[0] == 0:
            continue
        
        _ = self.model(img_batch_tensor)
        if len(features_blobs[0].shape)==4:
          out_batch = features_blobs[0].transpose(0,2,3,1)
        elif len(features_blobs[0].shape)==3:
          out_batch =features_blobs[0].transpose(0,2,1)
        features_blobs.pop()
  ...
  def _random_concept_activations(self, bottleneck, random_concept):
    ...
    if not os.path.exists(rnd_acts_path):
      rnd_imgs = self.load_concept_imgs(random_concept, self.max_imgs,other_dir=self.RANDOM_SOURCE)
  ...
  def _get_gradients(self, acts, y, bottleneck_name, example=None):
    ...
    if len(inputs.shape) == 4:
      outputs = cutted_model(inputs.permute(0,3,1,2).reshape(inputs.size(0), -1))
    elif len(inputs.shape) == 3:
      outputs = cutted_model(inputs.permute(0,2,1))

    #outputs = cutted_model(inputs.permute(0,3,1,2).view(inputs.size(0), -1))
  ...
  def get_acts_from_images(self, imgs):
    ...
    if len(features_blobs[0].shape) == 4:
      ...
      out=features_blobs[0].transpose(0, 2, 1)
    #out = features_blobs[0].transpose(0, 2, 3, 1)
    features_blobs.pop()
    return out
