...
def save_ace_report(cd, accs, scores, address):
  ...
  with open(address + 'ace_results.txt', 'w') as f:
    f.write(report)
...
def return_concepts(cd,scores):
  concepts={}
  for bn in cd.bottlenecks:
    concepts[bn]={}
    for concept in cd.dic[bn]['concepts']:
        concepts[bn][concept]={}
        concepts[bn][concept]['score']=np.mean(scores[bn][concept])
        concepts[bn][concept]['patches'] = (np.clip(cd.dic[bn][concept]['patches'], 0, 1) * 256).astype(
          np.uint8)
  return concepts
